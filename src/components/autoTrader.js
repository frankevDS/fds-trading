...
function calcPositionSize(balance, entryPrice, atr, riskPct = 0.02) {
  ...
  const stopPct    = (atr * 1.5) / entryPrice;
...
export function initAutoTrader(placeFn, getBalanceFn, getTradesFn) {
  liveRefs.placeTrade = placeFn;
  liveRefs.getBalance = getBalanceFn;
  liveRefs.getTrades  = getTradesFn;
}
...
async function runAutoScan() {
  ...
  for (const r of results) {
    ...
    const trade = {
      ...
      binanceSymbol: r.sym.binanceSymbol, autoConfidence: r.confidence,
    };

    liveRefs.placeTrade && liveRefs.placeTrade(trade);
    markTraded(r.sym.id);
    openAuto.push(trade);
